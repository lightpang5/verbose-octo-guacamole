		...
		<section id="mainright">
			<div id="matList">
				...
				<div class="container">
					...
					<table class="table table-striped" style="width: 850px; float: left; margin-left:44px;">
						...
						<tbody>
							...
							<tr class="picli" style="font-size: 15px; font-family: -webkit-pictograph; text-align:center;">
								<td><%=articleList.get(i).getGB_NUM()%><br>
								
								<a href="galDetail.bo?GB_NUM=<%=articleList.get(i).getGB_NUM()%>&page=<%=nowPage%>"> <img src="boardUpload/<%=articleList.get(i).getGB_FILE()%>" class="Fileimg"></a><br>
								<a href="galDetail.bo?GB_NUM=<%=articleList.get(i).getGB_NUM()%>&page=<%=nowPage%>" style="font-size:20px;"> <%=articleList.get(i).getGB_SUBJECT()%></a><br>
								 글쓴이:<%=articleList.get(i).getGB_ID()%><br> 작성일:<%=articleList.get(i).getGB_DATE()%><br> 조회수:<%=articleList.get(i).getGB_READCOUNT()%>
								</td>
							</tr>
					...
					<section id="pageList" class="pagelistposition">
						<%
							if (nowPage <= 1) {
						%>
						[이전]&nbsp;
						...
						%>
						<a href="galList.bo?page=<%=nowPage + 1%>">[다음]</a>
						<%
							}
						%>
					</section>
