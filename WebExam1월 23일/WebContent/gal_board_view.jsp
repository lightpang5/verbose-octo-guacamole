...
<%
	...
	ArrayList<G_BoardRepBean> articlelist = (ArrayList<G_BoardRepBean>) request.getAttribute("articlelist");
						...
						<div class="container" id="formInnerWrapper">
							...
							<table>
								<tr style="padding: 15px 15px 22px 15px;border-top: 2px solid #2c3c57;">
									...
									<td class="viewform" colspan="2" style="font-size:55px;padding-left: 40px;"><%=article.getGB_SUBJECT()%></td>
								</tr>
								<tr style="padding: 15px 15px 22px 15px; border-bottom: 1px solid #d5d8dd;padding-left: 40px;">
									<td class="viewsubject"><%=article.getGB_DATE() %></td>
									<td class="viewform"><%=article.getGB_ID()%></td>
								</tr>
								<tr>
									<td rowspan="2" class="viewsubject"  style="border-bottom: 2px solid #2c3c57;">내용</td>
									<td class="viewform" style="padding-top:50px;">
										<%
											if (!(article.getGB_FILE() == null)) {
										%>
										<a href="gal_file_down.jsp?downFile=<%=article.getGB_FILE()%>"> 
										<img alt="" src="boardUpload/<%=article.getGB_FILE()%>"  class="imgst">
										</a>
										<%
											}
										%>
							...
							<div id="rep">
								...
								<input type="hidden" name="GB_NUM" value="<%=article.getGB_NUM() %>" style="position:fixed;" />
								<input type="hidden" name="GB_ID" value="<%=article.getGB_ID() %>" style="position:fixed;" />
							...
							<div id="innerRep">
								...
								%>
										<tr>
											...
											<td id="repContents"><%=articlelist.get(i).getGB_REP_CONTENTS() %></td>
											<td id="repId"><%=articlelist.get(i).getGB_ID() %></td>
											<td id="repDate"><%=articlelist.get(i).getGB_REP_DATE() %></td>
										</tr>
								<% 
									}
								%>
							...
							<section id="commandList">
								<a href="galReplyForm.bo?GB_NUM=<%=article.getGB_NUM()%>&page=<%=nowPage%>"> [답변] </a> <a href="galModifyForm.bo?GB_NUM=<%=article.getGB_NUM()%>&GB_ID=<%=uss%>"> [수정] </a> <a href="galDeleteForm.bo?GB_NUM=<%=article.getGB_NUM()%>&page=<%=nowPage%>"> [삭제] </a> <a href="galList.bo?page=<%=nowPage%>">[목록]</a>&nbsp;&nbsp;
							</section>
