...
<%
	...
	//수정부분 끝
%>
<jsp:useBean id="dao" class="vo.QnaDao" />
<!doctype html>
<html lang="en">
<head>
<meta charset="UTF-8">
		...
		<section id="mainright">
			<div id="matList">
				<div class="sec_ban2">
					<ul class="nav_ban">
						<li><h3
								style="margin-bottom: 30px; margin-top: 5px; font-size: 50px; font-family: 'NanumBarunGothicL'; line-height: 60px; text-align: center;">고
								객 센 터</h3></li>
					</ul>
				</div>
				<div class="cbox1">
					<form id="qnainsert" action="qnaInsert.jsp" method="post">
									...
									<td>
										<div class="form-group has-warning"
											...
											<label class="control-label" for="inputWarning1"
												style="color: #8a6d3b;"> 글 수정,삭제,비밀글 확인시 사용됩니다. </label>
											<div class="checkbox" style="height: 25px;">
												<label for="qSecret2"> <input type="radio"
													id="qSecret2" name="q_b_secret" value="pb">공개
												</label> <label for="qSecret"> <input type="radio"
													id="q_b_secret" name="q_b_secret" value="sc" checked>비밀글
													설정
												</label>
											</div>
											<input type="password" class="form-control" id="q_b_secretcode"
												name="q_b_secretcode" maxlength="10"
												style="width: 250px; margin-top: 7px;">
										</div>
									</td>
								</tr>
						...
						<a href="qna.jsp" class="btn btn-default" style="float: right; margin-left: 5px;">
							목록 </a> <input type="submit" value="글쓰기"
							class="btn btn-default pull-right" style="margin-right: 20px;" /><br>
						<br> <br>
				</div>
				...
				<div class="cbox2">
					<table class="table table-striped">
						...
						<tbody>
							...
							<a>
								<tr>
									<td class="thgongji" style="text-align: left;"><%=list.get(i).getQ_b_no()%></td>
									<td class="thtitle"><a
										onClick="selectOne(<%=list.get(i).getQ_b_no()%>); "
										class="button" id="listtt"><%=list.get(i).getQ_b_title()%></a> <%
 									if (list.get(i).getQ_b_secret().equals("sc"))
 										out.print("<img src='img/lock2.png' class='secretimg' style='width: 15px; height: 15px;'>");
 								%></td>
									<td class="thname" name="thname" id="thname"><%=list.get(i).getQ_b_name()%></td>
									<td class="thdate"><%=list.get(i).getQ_b_date()%></td>
									<td class="thview"><%=list.get(i).getQ_b_readcount()%></td>
							...
							<%
								...
								<a>
									<tr>
										<td class="thgongji" style="text-align: left;"><%=searchlist.get(i).getQ_b_no()%></td>
										<td class="thtitle"><a class="button" id="listtt" onClick="selectOne(<%=searchlist.get(i).getQ_b_no()%>); ">
										<%=searchlist.get(i).getQ_b_title()%></a> 
										<%if (searchlist.get(i).getQ_b_secret().equals("sc"))
								 			out.print("<img src='img/lock2.png' class='secretimg' style='width: 15px; height: 15px;'>");%></td>
										<td class="thname" name="thname" id="thname"><%=searchlist.get(i).getQ_b_name()%></td>
										<td class="thdate"><%=searchlist.get(i).getQ_b_date()%></td>
										<td class="thview"><%=searchlist.get(i).getQ_b_readcount()%></td>
									</tr>
								</a>
								<%
									}
					...
					<select class="form-control"
						style="width: 200px; float: left; margin-left: 102px;">
						<option>전체기간
					</select> <select class="form-control" name="search" id="search"
						...
						onclick="searchselect()" style="float: left; margin-left: 10px;">
				</div>
		</section>
	</section>
